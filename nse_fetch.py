...
NSE_SNAPSHOT_URL = "https://www.nseindia.com/api/snapshot-capital-market-largedeal"
...
class NSESessionManager:
    ...
    def warm_up(self) -> bool:
        ...
        try:
            r1 = self.session.get(NSE_HOME_URL, headers=DEFAULT_HEADERS, timeout=12)
            time.sleep(1.0)
            r2 = self.session.get(NSE_LARGE_DEALS_PAGE, headers=DEFAULT_HEADERS, timeout=12)
...
def _parse_nse_date(val: Any) -> Optional[date]:
    ...
    # Handle ISO timestamp format (e.g. 2026-08-21T00:00:00.000Z)
    if "T" in val_str:
        try:
...
def normalize_nse_records(raw_items: List[Dict[str, Any]], deal_type_label: str) -> List[Dict[str, Any]]:
    ...
    for item in raw_items:
        ...
        date_raw = (
            ...
            or item.get("mTIMESTAMP")
            or ""
        )
        deal_date = _parse_nse_date(date_raw)
        if not deal_date:
            deal_date = date.today()  # Snapshot fallback defaults to today
...
def fetch_nse_deals(from_date: date, to_date: date) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    # 1. Query Historical Endpoints
    bulk_data, _ = mgr.safe_get_json(NSE_BULK_HIST_URL, params={"from": from_str, "to": to_str})
    bulk_items = []
    if isinstance(bulk_data, dict):
        bulk_items = bulk_data.get("data", []) or bulk_data.get("BULK_DEALS_DATA", [])
    elif isinstance(bulk_data, list):
        bulk_items = bulk_data

    block_data, _ = mgr.safe_get_json(NSE_BLOCK_HIST_URL, params={"from": from_str, "to": to_str})
    block_items = []
    if isinstance(block_data, dict):
        block_items = block_data.get("data", []) or block_data.get("BLOCK_DEALS_DATA", [])
    ...
    if len(bulk_items) == 0 and len(block_items) == 0:
        logger.info("NSE historical returned 0 records. Trying snapshot endpoint...")
        snap_data, _ = mgr.safe_get_json(NSE_SNAPSHOT_URL)
        if isinstance(snap_data, dict):
            bulk_items = snap_data.get("BULK_DEALS_DATA", []) or snap_data.get("data", [])
            block_items = snap_data.get("BLOCK_DEALS_DATA", [])
    ...
    # Date range filter with snapshot tolerance
    if not status_info["used_snapshot_fallback"]:
        df = df[(df["date"] >= from_date) & (df["date"] <= to_date)]

    status_info["cleaned_count"] = len(df)
    return df, status_infoAPI_HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
    "Accept": "application/json, text/plain, */*",
    "Accept-Language": "en-US,en;q=0.9",
